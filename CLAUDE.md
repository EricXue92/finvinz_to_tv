...
**Six screener groups with different output behavior:**
...
- **Cross-day master dedup (`output/state/eod_seen_{US,HK,IPO}.txt`)**: Applied to the **long-side** EOD groups (5 Longs splits, Leaders, RS) **after** the within-day priority dedup. The master file is a per-market append-only set of every ticker ever written by the long-side pipeline. Each group's daily output = its within-day survivors **minus** the master; surviving "new" tickers are then added to the master. Net effect: a given long-side ticker enters exactly one of those 7 EOD groups on its first sighting and never re-appears in any long-side `.txt` / Webull / Futu push afterward. Implemented in `_dedup_seen` (modeled on `_morning_gap_new_tickers`). Reset by deleting the file — manual only. **Shorts, HK Shorts, and MorningGap are excluded** from this flow. Shorts/HK Shorts are intentionally NOT deduped against past runs — short setups are time-sensitive (parabolic blow-off, multi-week-extension); the same ticker can re-qualify weeks later and that re-detection is meaningful. They also do NOT write to the master, so a Shorts hit today doesn't suppress a future Longs hit on the same ticker. MorningGap has its own per-day seen file at `output/state/morning_gap_seen_<date>.txt`. **IPO has its own master `eod_seen_IPO.txt`**, independent of `eod_seen_US.txt` — a ticker collected as an IPO drop today still lands in its proper long-side group on the first day it has enough yfinance history to pass DV/ADR/RVol. The long-side Futu groups are listed in `[futu] append_only_groups` so the Futu side accumulates monotonically too — the daily date-stamped `.txt` records "today's NEW additions", while the Futu group records the all-time union. Shorts/HKShorts are still append-only on the Futu side (group accumulates), but the daily .txt now contains every Finviz-detected short for the day, including re-detections.
