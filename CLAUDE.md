...
- **Longs** (`[[longs]]` in config): 5 strategies, each written to its **own** file (no merged `Longs.txt`). Config list order = priority for internal mutual-exclusion dedup: `EarningsGap > HighVolume (Relative Volume Surge) > GapUp > NewHigh52W > TopGainers` → `output/TV/US/{EarningsGap,HighVolume,GapUp,NewHigh52W,TopGainers}.txt`. Each `[[longs]]` entry has a `key` field; the matching `longs_<key>` entry under `[futu.groups]` supplies both the Futu group name and the .txt filename stem. The union of all 5 acts as a "virtual Longs" for the cross-group `Longs > Leaders` dedup (RS is excluded — see Cross-group dedup bullet below). Based on Oliver Kell's methodology. HighVolume uses yfinance post-processing for 20-day relative volume (configurable via `min_relative_volume` and `relative_volume_days`).
...
- **RS** (`[rs]`): Conditional → `output/TV/US/RS.txt`. Only runs when both SPY and QQQ drop >=1.2% (checked via `finviz.get_stock()`). Based on Oliver Kell's relative strength approach.
- **IPO** (no config; auto-collected sidecar of the long-side pipeline): Tickers that pass Finviz long-side screeners (Longs/Leaders/RS) but get dropped by yfinance for missing/insufficient daily history. Sent through a depth-conditional ladder (mirror of HK `filter_hk_ipo_candidates`, implementation in `us_ipo.filter_us_ipo_candidates`) → `output/TV/US/<date>_IPO.txt`:
  - `len(df) < 20` → dropped (`drops['min_history']`). Fresh IPOs in their first 19 days are too noisy on volume.
  - **≥ 20 days** (always): cap ≥ $300M (from Finviz cap captured during screener pass via `run_screener(..., capture_caps=...)`), price ≥ $10.
  - **≥ 20 days** (with 20-day metrics): avg_vol ≥ 500K, $vol ≥ $100M, ADR ≥ 4.0%.
  - **≥ 50 days**: above SMA50.
  - **≥ 200 days**: above SMA200.
  - **≥ 64 days** (3M RS algorithm can score the ticker): 3M RS percentile ≥ 90. Percentile derived by computing the IPO's `WEIGHTS_3M` score vs SPY and ranking it against the long-side 3M table's `raw_score` distribution — i.e., "where would this IPO rank if it joined the Fred6725 universe today". When `rs_table_3m is None` (3M layer disabled or fetch failed) the RS gate is skipped entirely.

  Has its own append-only Futu group `IPO` and cross-day master `output/state/eod_seen_IPO.txt` — independent of `eod_seen_US.txt`, so a ticker that ages into having enough yfinance data still lands in its proper long-side group on the first qualifying day. ADR%/dollar-volume rejections from the long-side flow are NOT IPO drops (they're real filter rejections, not data gaps).
...
- **HK Long-side** (`[hk_settings]` + `[[hk_longs]]` + `[[hk_leaders]]` + `[hk_rs]`): Five strategies sourced from yfinance (k-line + HSI) plus Futu (market caps + live HSI day-change). The original spec was Futu-only, but Futu's free/Lv1 tier capped 12-month history coverage at ~12% of the Main Board universe (PR #7 diagnostic), so the IBD 12-month RS algorithm had nothing to rank. yfinance reliably gives 2+ years for nearly every Main Board listing. Universe = HKEX Main Board equities (~2,400). All five share a baseline: cap ≥ HK$300M, avg vol ≥ 500K shares/day, $vol ≥ HK$100M, ADR ≥ 3.5%, price ≥ HK$20, **above SMA50 & SMA200** (mirrors US Longs `ta_sma50_pa` + `ta_sma200_pa` on every filter set), RS ≥ 90 (vs HSI). Within-day cross-strategy priority: `EarningsGap > HighVolume > GapUp > Leaders` (RS is excluded from this dedup — see Cross-group dedup bullet below). Each writes to its own `output/TV/HK/<date>_<Name>.txt`:
  ...
  - **HK RS** — conditional. Only runs when HSI day-change ≤ −1.2% (Futu snapshot of `HK.800000`). No additional gates beyond the universal baseline. Output: `output/TV/HK/<date>_RS.txt`.
- **HK IPO** (no config; auto-collected sidecar of the HK long-side pipeline): tickers in the HKEX Main Board universe that yfinance returned but with `< 253 rows of daily close` (insufficient for the IBD 12-month RS calc) — almost always fresh HK IPOs that aged into yfinance but haven't accumulated 12 months of data. **Partially RS-gated**: tickers with ≥ 64 trading days must clear the 3M RS gate; shorter-history tickers go through the metric ladder only. Filters apply **conditionally** based on history depth so a 30-day-old IPO still surfaces while a 200-day-old IPO is held to nearly the full long-side baseline. Implementation: `filter_hk_ipo_candidates(klines, metrics, rs_table_3m, hk_settings)` in `hk_eod.py` (pure function, unit-tested).
  ...
  - **Always** (≥ 20 days): cap ≥ HK$300M, price ≥ HK$20.
...
- **Cross-day master dedup** (`output/state/eod_seen_{US,HK,IPO}.txt`, implemented in `_dedup_seen`): applied to first-sighting long-side EOD groups (5 US Longs splits + Leaders; 4 HK long-side groups: EarningsGap/HighVolume/GapUp/Leaders) AFTER within-day priority dedup. Each daily output = within-day survivors **minus** master; new survivors append to master. Net effect: every first-sighting long-side ticker enters exactly ONE of its market's groups on first sighting and never reappears in any of those `.txt` / Webull / Futu pushes. Reset by deleting the file (manual only). **Markets are independent**: `eod_seen_US.txt` and `eod_seen_HK.txt` never cross-contaminate — a ticker dual-listed in both markets (rare for our universe) would track separately.
  ...
  - **Excluded**: US Shorts, HK Shorts, **US RS, HK RS**, Morning Gap. Short setups are time-sensitive (parabolic blow-off can re-qualify weeks later), so re-detection is meaningful and a Shorts hit today does NOT suppress a future Longs hit on the same ticker. RS is conditional on weak-market days (SPY+QQQ ≤ −1.2% / HSI ≤ −1.2%); the whole point of the scan is to surface strong names regardless of whether they've been collected before, so it bypasses both within-day priority dedup and the cross-day master. An RS hit today does NOT suppress a future Longs/Leaders hit on the same ticker, and vice versa. Morning Gap uses its own per-day, per-phase seen files `output/state/morning_gap_seen_{pre,post}_<date>.txt` — pre and post are independent so a pre-market push does not suppress a post-open push (and vice versa); see the Morning Gap entry above for promotion-alert behavior.
...
- **Retention cleanup** (`cleanup.cleanup_old_outputs`): After each successful scan run (`us-eod`, `hk-eod`, `morning-gap`, `hk-morning-gap`), dated output artifacts older than yesterday are deleted. Covers `output/TV/{US,HK}/`, `output/Webull/{US,HK}/`, `output/Reports/`, and the dated state files (`morning_gap_seen_{pre,post}_*`, `hk_morning_gap_seen_post_*`, `hk_rs_rating_*.csv`). `rs_rating_*.csv` gets a 4-day window instead to preserve the documented 3-day fetch fallback in `rs_rating.py` (`_FALLBACK_MAX_AGE_DAYS = 3`). The cross-day master dedup files (`eod_seen_{US,HK,IPO,HKIPO}.txt`), `ntfy_last_seen.txt`, `edgar_cache/`, and `launchd_*.log` are **never** touched. Cleanup is glob-driven (unrecognised filenames are preserved) and soft-fails — a cleanup error logs a warning and the run still exits 0. Outside-window early-exits in morning-gap modes skip cleanup (nothing was written that run).
...
### US: 12M (Fred6725 CSV, vs SPY) + 3M (Leaders/RS/Shorts only, vs SPY)

`rs_rating.py` pulls the daily IBD-style RS percentile table (0-99) from `Fred6725/rs-log/output/rs_stocks.csv` (the published artifact of the [Fred6725/relative-strength](https://github.com/Fred6725/relative-strength) GitHub Action) and exposes a filter applied to gated EOD groups **right after `run_screener`** (or after the Finviz Ownership screener for Shorts) — placed before any yfinance batch download so the gate cuts most tickers before the expensive step. **Longs (5 splits)** gate at 12M RS ≥ 90 only. **Leaders + conditional RS group + US Shorts** gate at both 12M ≥ 90 and 3M ≥ 90 (double gate). HK Shorts, Morning Gap, and IPO are NOT RS-gated.

**3M layer (added 2026-05-21).** `us_rs_3m.py` computes a second IBD-style percentile locally using `WEIGHTS_3M = 0.5·R21 + 0.3·R42 + 0.2·R63` vs SPY (yfinance). Universe = Fred6725 12M table's tickers (~6100). Cached to `output/state/rs_rating_3m_<date>.csv` with both `rs_percentile` and `raw_score` columns — `raw_score` is used by the IPO ladder to percentile-rank out-of-universe candidates against the same Fred6725 distribution via `np.searchsorted`.

The 3M gate applies to **Leaders + conditional RS group + Shorts** only. **Longs 5 splits stay 12M-only** — they already have strong event filters (EarningsGap / RVol surge / GapUp / 52-week high / Top Gainer) and stacking 3M would over-tighten the universe. Both layers use threshold ≥ 90 by default (`[settings] min_rs_percentile_3m = 90`); set to 0 to disable the 3M layer entirely (yfinance batch is skipped on disable).
...
- **Config**: `[settings] min_rs_percentile = 90` (Leaders 12M) and `min_rs_percentile_longs = 90` (Longs 5 splits + RS group + US Shorts 12M) and `min_rs_percentile_3m = 90` (Leaders + RS group + US Shorts 3M double gate). Set any to 0 to disable that tier independently; the GitHub fetch is skipped only when both `min_rs_percentile` and `min_rs_percentile_longs` are 0; the yfinance 3M batch is skipped when `min_rs_percentile_3m` is 0 AND the IPO ladder has no tickers with ≥ 64 days of history.
- **Scope**: All US long-side EOD groups plus US Shorts gate at 12M RS ≥ 90. **Leaders**, the conditional **RS** scan, and **US Shorts** additionally gate at 3M RS ≥ 90 (double gate). **Longs (5 splits)** (EarningsGap, HighVolume, GapUp, NewHigh52W, TopGainers) are 12M-only — strong event filters make an additional 3M gate over-restrictive. **HK Shorts** is unfiltered (HK tickers aren't in the US RS table; gating would either drop everything or — under the keep-as-missing policy — be a no-op). **Morning Gap** is unfiltered (intraday discovery). **IPO** uses a conditional 3M gate only (≥ 64 days of history; see IPO ladder above).
