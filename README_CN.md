...
自动化股票筛选工具，运行自定义 Finviz 筛选（美股）和 HKEX + yfinance 筛选（港股），并将结果导出为 TradingView 可导入的观察列表。
...
| 涨幅榜 | 平均成交量 > 50万, 股价 > $20, Beta > 1.5, 高于 SMA200, 信号: 涨幅榜 |
| 跳空高开 | 平均成交量 > 50万, 股价 > $20, Beta > 1.5, 跳空高开 3%+, 高于 SMA200 |
| 52周新高 | 小盘股+, 平均成交量 > 100万, 股价 > $20, Beta > 1.5, 52周新高, 高于 SMA50 和 SMA200 |

所有做多策略还要求**美元成交量 >= $1亿**（股价 × 20日均量，通过 yfinance 计算）。上表中的"平均成交量"为 Finviz 3个月均量预筛选，用于减少后处理数据量。
...
| 筛选条件 | 标准 |
|---------|------|
| 美元成交量 | 股价 × 20日均量 >= $1亿（通过 yfinance） |
| 涨幅（大盘股 ≥ $100亿） | 2、3 或 4 周涨幅 50%+ |
| 涨幅（中盘股 $20亿–$100亿） | 2、3 或 4 周涨幅 200%+ |
| 涨幅（小盘股 $3亿–$20亿） | 2、3 或 4 周涨幅 300%+ |
| 连续上涨天数 | 连续 3 天以上收阳（通过 yfinance 获取数据；盘中运行时排除当日未完成数据，收盘后包含当日） |

涨幅先通过 Finviz 检查 4 周窗口，未通过的再通过 yfinance 检查 2 周和 3 周窗口，结果合并。

### RS - 相对强度（条件触发）

基于 **Oliver Kell** 的相对强度方法。仅在 SPY 和 QQQ 同日跌幅超过 1.5% 时运行——在弱势市场中寻找表现强势的股票。
...
港股做空候选，使用与美股做空相同的方法论，数据来源为 **HKEX + yfinance**。

**第一阶段 — HKEX 股票池 + yfinance 筛选：**

| 筛选条件 | 标准 |
|---------|------|
| 股票池 | HKEX 主板股票（约 2,400 只） |
| SMA20 | 股价高于 20 日均线 20%+ |
| 平均成交量 | > 100万股/日（20日均量） |
...
| 市值 | >= 3亿港元 |
| 成交额 | 股价 × 20日均量 >= 1亿港元 |
| 涨幅（大盘股 ≥ 100亿港元） | 2、3 或 4 周涨幅 50%+ |
| 涨幅（中盘股 20亿–100亿港元） | 2、3 或 4 周涨幅 200%+ |
| 涨幅（小盘股 3亿–20亿港元） | 2、3 或 4 周涨幅 300%+ |
| 连续上涨天数 | 连续 3 天以上收阳 |

港股代码以 `HKEX:XXXX` 格式输出（如 `HKEX:0700`），可直接导入 TradingView。
...
每次运行会同时生成最新文件（如 `Shorts.txt`）和带日期的归档副本以保留历史记录。文件内容为逗号分隔的股票代码，可直接导入 TradingView。
...
2. 右侧面板 -> 观察列表 -> 点击列表名称
3. 选择"导入列表..."
4. 选择 `output/US/Longs.txt`（或 Shorts/RS，港股选择 `output/HK/Shorts.txt`）
...
- [yfinance](https://github.com/ranaroussi/yfinance) - Yahoo Finance 数据，用于后处理筛选及港股数据
- [openpyxl](https://openpyxl.readthedocs.io/) - HKEX 证券列表 xlsx 解析
